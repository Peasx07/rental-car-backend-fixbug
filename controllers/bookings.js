...
exports.updateBooking = async (req, res, next) => {
    try {
        ...
        if (req.body.pickUpDate && req.body.dropOffDate) {
            ...
            // 1. เช็คห้ามแก้วันที่ให้เป็นวันในอดีต (เหมือน addBooking)
            if (pDate < today) {
                return res.status(400).json({ 
                    success: false, 
                    message: 'วันรับรถต้องไม่เป็นวันที่ในอดีต' 
                });
            }

            // 2. เช็คว่าวันคืนรถต้องอยู่หลังวันรับรถ (สิ่งที่คุณต้องการ)
            if (dDate <= pDate) {
                return res.status(400).json({ 
                    success: false, 
                    message: 'วันคืนรถต้องอยู่หลังจากวันรับรถ' 
                });
            }
            
            // 3. เช็คการทับซ้อน (โค้ดเดิมที่มีอยู่แล้ว)
            const overlappedBooking = await Booking.findOne({
